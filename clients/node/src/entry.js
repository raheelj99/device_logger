...
// I/O and protobuf runtime so they are trivially unit-testable; proto.js turns
...
function topicFor(deviceId, subsystem) {
...
function buildSanitizationJob(deviceId, traceId) {
  const media = {
  ...
  let seq = 0;
  const step = (phase, message, extra = {}) =>
    buildLogEntry({
      deviceId,
      seq: ++seq,
      severity: Severity.INFO,
